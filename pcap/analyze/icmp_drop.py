...
MAX_DIFF_TIME = 10
PCAP_DIR = "/root/pcap/"
WGET_PATH = "http://192.168.17.20/everoute/dev/zwtop/20250902/tcpdump/"
NODE_MAP = {
    "10.255.0.98": "em1-10.255.0.98.pcap",
...
def wget_pcap():
    ip = "10.255.0.104"
    pcap_file = PCAP_DIR + ip + ".pcap"
    if not os.path.exists(pcap_file):
        for i in range(0,10):
            index = "%d"%i
            if NODE_MAP[ip] is None:
                print(f"node ip {ip} is invalid")
                raise Exception("node ip is invalid")
            pcap = NODE_MAP[ip] + index
            if os.path.exists(PCAP_DIR + pcap):
                continue
            pcap_wget_cmd = "wget -P " + PCAP_DIR + " " + WGET_PATH + pcap
            res = os.system(pcap_wget_cmd)
            if res != 0:
                print(f"cmd {pcap_wget_cmd} failed, errcode: {res}")
                raise Exception("exec cmd failed")
        mcap_cmd = " mergecap -w " + pcap_file + " " + PCAP_DIR + NODE_MAP[ip] + "*"
        res = os.system(mcap_cmd)
        if res != 0:
            print(f"cmd {mcap_cmd} failed, errcode: {res}")
            raise Exception("exec cmd failed")
    for ip in ["10.255.0.98", "10.255.0.99", "10.255.0.143"]:
        pcap_file = PCAP_DIR + ip + ".pcap"
...
def process2(srcip, dstip, drop_csv):
    wget_pcap()
    src_pcap, dst_pcap = prepare_pcap2(srcip, dstip)
    analyze_pcap_diff(
            ori_file_name="invalid",
            srcip=srcip,
            dstip=dstip,
            source_pcap=src_pcap,
            target_pcap=dst_pcap,
...
def prepare_pcap2(srcip, dstip):
    src_pcap, dst_pcap = gen_pcap_file_name(srcip, dstip, "")
